...
export function ClassStudentManagement({ course, students, loading, error, onBack, onRoster, onToast, onRetry }: {
  ...
  return <>
    ...
    <header className="ui-panel border-l-4 border-l-blue-600 p-5 sm:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="font-mono text-xs font-extrabold text-blue-700">{course.code}</p>
          <h1 id="classroom-title" className="mt-2 text-3xl font-bold text-slate-950">{course.title}</h1>
        ...
        <button type="button" onClick={() => setAddOpen(true)} className="ui-button-primary w-full sm:w-auto">+ Thêm sinh viên</button>
      </div>
    </header>
    ...
    {editing ? <ModalDialog title="Sửa sinh viên trong lớp" onClose={() => !editBusy && setEditing(null)}><div className="p-5 sm:p-6"><label className="block text-sm font-semibold text-slate-700">Tên hiển thị trong lớp<input value={editName} maxLength={100} onChange={(event) => setEditName(event.target.value)} className="ui-control mt-2 font-normal" /></label><p className="mt-2 text-xs text-slate-500">Tên này chỉ được sử dụng trong lớp, không thay đổi tài khoản của sinh viên.</p><div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={() => setEditing(null)} disabled={editBusy} className="ui-button-secondary">Hủy</button><button type="button" onClick={() => void saveStudentName()} disabled={editBusy || !editName.trim()} className="ui-button-primary">{editBusy ? 'Đang lưu...' : 'Lưu thay đổi'}</button></div></div></ModalDialog> : null}
...
function EmptyRoster({ onAdd }: { onAdd: () => void }) { return <div className="ui-state mt-4"><div><h2 className="text-lg font-bold text-slate-900">Chưa có sinh viên</h2><p className="mt-2 text-sm text-slate-500">Hãy thêm sinh viên vào lớp để bắt đầu quản lý.</p><button type="button" onClick={onAdd} className="ui-button-primary mt-5">+ Thêm sinh viên</button></div></div> }
